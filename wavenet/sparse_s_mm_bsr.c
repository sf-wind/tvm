...
clang -m64  -w -DMKL_ILP64 -I"/opt/intel/compilers_and_libraries_2019.3.199/mac/mkl/include" \
        ...
        -Wl,-rpath,/opt/intel/compilers_and_libraries_2019.3.199/mac/mkl/lib -Wl,-rpath,/opt/intel/compilers_and_libraries_2019.3.199/mac/mkl/../compiler/lib \
         -lpthread -lm -o sparse_s_mm_bsr.out
*/
...
const int BN = (N / B);
const int BK = (K / B);
const int BNNZ = (int)(125385 / (B * B));
const int NNZ = BNNZ * B * B;
#define ALIGN 512
...
/* Allocation of memory */
    ...
    columns_A = (MKL_INT *)mkl_malloc(sizeof(MKL_INT) * BNNZ, ALIGN);
    rowIndex_A = (MKL_INT *)mkl_malloc(sizeof(MKL_INT) * (BN + 1), ALIGN);

    /* K * M */
    x = (float *)mkl_calloc(K * M, sizeof( float ), DENSE_ALIGN);

    /* N * M */
    y = (float *)mkl_calloc(K * N, sizeof(float), DENSE_ALIGN);
...
/* Set values of the variables*/
 ...
 //Matrix A
    ...
    rowIndex_A[0] = 0;
    for( i = 1; i < BN + 1; i++ ) {
      int next = rowIndex_A[i - 1] + (int)(BNNZ / BN + 1);
      next = next > BNNZ- 2 ? BNNZ- 2 : next;
      rowIndex_A[i] = next;
    }
    rowIndex_A[BN] = BNNZ;
    /*
...
/* Prepare arrays, which are related to matrices.
   Create handles for matrices A and B stored in CSR format */
    CALL_AND_CHECK_STATUS(mkl_sparse_s_create_bsr( &csrA, SPARSE_INDEX_BASE_ZERO,
        SPARSE_LAYOUT_ROW_MAJOR, BN, BK, B, rowIndex_A, rowIndex_A+1, columns_A, values_A ),
                          "Error after MKL_SPARSE_S_CREATE_BSR, csrA \n");
    //printf("test1\n");
/* Compute C = A * B  */
    for (int i = 0; i < 10; i++) {
      for (int j = 0; j < M * K; j++) {
